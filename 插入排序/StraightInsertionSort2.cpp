...
int main(){
    ...
    StraightInsertionSort2(IntegerArray,CurrentSize);
...
//函数：StraightInsertionSort2
//作用：将array里的数据进行二分插入排序，size为数组长度
void StraightInsertionSort2(int array[],int size){
    int temp;
    for(int i=1;i<size;i++){
        ...
        if(array[i]<array[i-1]){
            ...
            while(low<=high){
                mid=(high-low)/2+low;
                if(array[mid]<array[i])  low=mid+1;
                else  high=mid-1;
            }
            for(j=i-1;j>=low;--j){         // 或者j>=high+1，这里while循环之后low=high+1，插入的位置即为low
                array[j+1]=array[j];
            }
            array[low]=temp;     //array[high+1]=temp
        }
    }
}
